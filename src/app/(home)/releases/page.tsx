  ...
  return (
    <main className="relative h-full w-full">
      ...
      <div className="bg-[radial-gradient(125%_125%_at_50%_10%,_#ffffff_40%,_#ffcccc_100%)] dark:bg-[radial-gradient(125%_125%_at_50%_10%,_#000000_40%,_#2b0707_100%)] rounded-xl border py-12 md:px-8">
        <h1 className="mb-2 text-3xl font-bold">Latest Releases</h1>
        <p className="mb-4 text-fd-muted-foreground">Our official releasess page.</p>
          <Link
            href="/"
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {posts.map((post) => (
          ...
          >
            ...
            <p className="text-sm text-fd-muted-foreground">{post.data.version} &bull; {new Date(post.data.date).toDateString()}</p>
            </div>
          </Link>
        ))}
      </div>
