...
export default function Page() {
  ...
  const allIcons: IconEntry[] = useMemo(() => {
    const outlineEntries: IconEntry[] = Object.entries(OutlineIcons).map(([name, Comp]) => ({
      name,
      Component: Comp as any,
      variant: "outline" as const,
    }));

    const solidEntries: IconEntry[] = Object.entries(SolidIcons).map(([name, Comp]) => ({
      name,
      Component: Comp as any,
    ...
    <main className="relative h-full w-full">
      ...
      <div className="bg-[radial-gradient(125%_125%_at_50%_10%,_#ffffff_40%,_#ffcccc_100%)] dark:bg-[radial-gradient(125%_125%_at_50%_10%,_#000000_40%,_#2b0707_100%)] rounded-xl border py-12 md:px-8">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center bg-zinc-100 dark:bg-zinc-800 rounded-lg p-1 shadow-sm">
              <button
                ...
                className={`px-3 py-1 rounded-md text-sm font-medium transition ${variant === "outline" ? "bg-white dark:bg-zinc-700 shadow" : "text-zinc-600 dark:text-zinc-300"}`}
              >
                Outline
              </button>
              <button
                onClick={() => setVariant("solid")}
                className={`px-3 py-1 rounded-md text-sm font-medium transition ${variant === "solid" ? "bg-white dark:bg-zinc-700 shadow" : "text-zinc-600 dark:text-zinc-300"}`}
              >
                Solid
              </button>
            </div>
        ...
        <div className="mt-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3 flex-1">
            <div className="relative flex-1">
              ...
              />
              <div className="absolute right-3 top-1/2 -translate-y-1/2 text-zinc-400 text-sm">{filtered.length} icons</div>
            </div>
          </div>
        </div>
        ...
        <section className="p-2 bg-gradient-to-b from-fd-background/80 to-fd-background border-t border-b rounded-xl border-black/20 dark:border-zinc-800 mt-2">
            ...
            {filtered.map(({ name, Component }) => (
              ...
              >
                <div className="flex items-center justify-center w-16 h-16 rounded-lg bg-zinc-50 dark:bg-zinc-800/60 shadow-sm">
                  <Component className="w-8 h-8 text-zinc-700 dark:text-zinc-100" aria-hidden />
                </div>

                <div className="text-xs text-center text-zinc-600 dark:text-zinc-300 break-words">{ name.length > 18 ? name.slice(0, 18) + '..' : name }</div>
          ...
          {filtered.length === 0 && (
            <div className="mt-8 text-center text-zinc-500 dark:text-zinc-400">No icons match your search.</div>
          )}
        </section>
