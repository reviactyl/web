...
<Tabs items={['Nginx With SSL', 'Nginx Without SSL', 'Apache With SSL', 'Apache Without SSL', 'Caddy With SSL', 'Caddy Without SSL']}>
...
Now, you should paste the contents of the file below, replacing `<domain>` with your domain name being used in a file called `panel.conf` and place the file in `/etc/nginx/sites-available/`, or — if on RHEL, Rocky Linux, or AlmaLinux, `/etc/nginx/conf.d/`.
```nginx 
server {
