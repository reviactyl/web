...
## Creating a database for Reviactyl
MySQL is a core component of Reviactyl Panel but it can be confusing to setup and use if you've never done so before.
...
Next, we will create a user called `reviactyl` and allow logins from localhost which prevents any external connections
...
CREATE USER 'reviactyl'@'127.0.0.1' IDENTIFIED BY 'somePassword';
```
...
Finally, we need to tell MySQL that our reviactyl user should have access to the panel database. To do this, simply
...
CREATE USER 'reviactyluser'@'127.0.0.1' IDENTIFIED BY 'somepassword';
```
...
```sql
GRANT ALL PRIVILEGES ON *.* TO 'reviactyluser'@'127.0.0.1' WITH GRANT OPTION;
```
