...
- PHP `8.2`, `8.3`, `8.4` or `8.5` (recommended) with the following extensions: `cli`, `openssl`, `gd`, `mysql`, `PDO`, `mbstring`, `tokenizer`, `bcmath`, `xml` or `dom`, `curl`, `zip`, `intl`, and `fpm` if you are planning to use NGINX.
...
apt -y install php8.5 php8.5-{common,cli,gd,mysql,mbstring,bcmath,xml,fpm,curl,zip,intl,redis,pgsql} mariadb-server nginx tar unzip git redis-server postgresql postgresql-contrib
```
