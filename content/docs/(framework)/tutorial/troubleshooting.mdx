...
* Check that Agent is running, and not reporting errors. Use `systemctl status agent` to check the current status of
  the process.
...
* Make sure Agent is properly installed and the active configuration matches the configuration shown under
`Admin -> Node -> Configuration` in the Panel.
* Check that the Agent ports are open on your firewall. Agent uses ports `8080` or `8443` for HTTP(s) traffic,
and `2022` for SFTP traffic.
* Ensure you have AdBlock disabled or whitelisted for your Panel and Agent domains.
* Check that the Panel can reach Agent using the domain that is configured on the Panel. Run `curl
https://domain.com:8080` on the Panel server and ensure that it can successfully connect to Agent.
* Ensure that you are using the correct HTTP scheme for your Panel and Agent. If the Panel is running over HTTPS
  Agent will also need to be running on HTTPS.
* If using HTTPS for Agent, make sure that the certificates have not expired.

### More Advanced Debugging Steps

* Stop Agent and run `agent --debug` to see if there are any errors being output. If so, try resolving them manually,
  or reach out on [Discord](/discord) for more assistance.
* Check your DNS and ensure that the response you receive is the one you expect using a tool such as `nslookup` or `dig`.
* If you use CloudFlare make sure that the orange cloud is disabled for your Agent or Panel `A` records.
* Make sure when using Agent behind a firewall — pfSense, OpenSwitch, etc. — that the correct NAT settings to access
...
* When running the Panel and Agent on one server it can sometimes help if to add an entry in `/etc/hosts` that directs
...
* When running Agent and the Panel on separate VM's using the same adapter make sure the VM's can connect to each
...
Now that Agent has run successfully and you have gotten the green heart on your Nodes page, the agent config at '/etc/reviactyl/config.yml' will have new values.
...
```bash
# Network Manager (This will show both your IPV4 DNS and IPV6 DNS Servers in case you want to add the IPV6 DNS Server(s) from your host to your Agent Config as well.
...
If this returns different DNS Servers than 1.1.1.1 and 1.0.0.1 you'll need to edit the agent 'config.yml' file to use the DNS servers that were returned from the command. If you see output that looks like an IPV6 address in addition to your IPV4 DNS Servers, make sure you put that in the IPV6 section and not the IPV4 section. To be clear, if you have to use different DNS Servers than the default, make sure to REMOVE 1.1.1.1 and 1.0.0.1 from the agent config; don't just add the new servers, replace the old servers.

## Schedule Troubleshooting

- Check logs from your queue manager `journalctl -xeu pteroq`
- Restart pteroq `systemctl restart pteroq`
- Clear schedule cache `php /var/www/reviactyl/artisan schedule:clear-cache`
...
- Are your tasks off by a bit? Make sure you on the latest version of the panel? In version 1.11.5 there was a fix for schedules running at the wrong time. Alternatively, you may have the wrong timezone set. Make sure your timezones all match.
  ...
  - Panel Timezone `nano /var/www/reviactyl/.env`
  - Agent Timezone (Passed to containers as the TZ environmental variable, unrelated to schedules but while you're checking timezones you may as well set this too) `nano /etc/reviactyl/config.yml`
...
- Check for panel errors `tail -n 150 /var/www/reviactyl/storage/logs/laravel-$(date +%F).log | nc pteropaste.com 99`
...
```bash
firewall-cmd --permanent --zone=trusted --change-interface=reviactyl0
firewall-cmd --reload
```

Restart `docker` and `agent` after running these to be sure the rules are applied.
