...
<Callout type="warn">
These are advanced configurations for Agent. You risk breaking Agent and making containers unusable if
you misconfigure something. Proceed only if you know what each configuration value does.
</Callout>

You must apply all changes to your Agent `config.yml` file located at `/etc/reviactyl` and restart agent. Verify your config file using [Yaml Lint](http://www.yamllint.com/) should you receive errors related to YAML parsing.
...
You can change the network interface that Agent uses for all containers by editing the network name; it is by default set to `reviactyl_nw`. For example, to enable Docker host mode change the network name to `host`.

<Callout type="warn">
Changing network mode to `host` grants Reviactyl direct access to all machine interfaces and Panel users can bind to any IP or Port even if it's not allocated to their container. You will lose all benefits of Docker network isolation. It is not recommended for public installations that are hosting other users' servers.
</Callout>
...
After making changes, the following commands will stop the Agent, remove the Reviactyl network, and start the Agent again. Run at your own risk.
`systemctl stop agent && docker network rm reviactyl_nw && systemctl start agent`

## Enabling Cloudflare proxy

Cloudflare proxying of the Agent isn't beneficial since users will be connecting to the machine directly and bypassing any Cloudflare protection. As such, your Node machine IP will still be exposed.

To enable Cloudflare proxy, you must change the Agent port to one of the Cloudflare HTTPS ports with caching enabled (more info [here](https://developers.cloudflare.com/fundamentals/get-started/reference/network-ports/)), such as 8443, because Cloudflare only supports HTTP on port 8080. Select your Node in the Admin Panel, and on the settings tab, change the port. Make sure that you set "Not Behind Proxy" when using Full SSL settings in Cloudflare. Then on Cloudflare dashboard, your FQDN must have an orange cloud enabled beside it.
...
Restart agent and your game server to apply the new limit.
...
More commonly discussed values. View all Agent config values and explanations in [these two files.](https://github.com/reviactyl/agent/tree/develop/config)

| Setting Key                | Default Value | Notes                                                                                                                                                      |
| -------------------------- | :-----------: | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| debug                      |     false     | Force Agent to run in debug mode                                                                                                                           |
...
| app_name                   | "Reviactyl" | Changes the name of the daemon, shown in the panel's game console                                                                                          |
