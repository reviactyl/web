...
Agent is the next generation server control plane from Reviactyl. It is the fork of Pterodactyl wings with additional features.

<Callout type="warning">
You should only install Agent if you are running **Reviactyl v26.x**. <br />
Installing this software on previous versions is not recommended.
</Callout>
...
|                                    | 12      | Fully Supported |                                                             |
|                                    | 13      | Fully Supported |                                                             |
| **Windows**                        | All     | Not Supported   | This software will not run in Windows environments.         |
...
![configuration](/docs/configuration-wings.png)
...
To start Agent, simply run the command below, which will start it in a debug mode. Once you confirmed that it is running without errors, use `CTRL+C` to terminate the process and daemonize it by following the instructions below. Depending on your server's internet connection pulling and starting Wings for the first time may take a few minutes.
