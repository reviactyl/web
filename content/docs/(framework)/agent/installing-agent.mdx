...
Agent is the next generation server control plane from Reviactyl. It is the fork of Pterodactyl agent with additional features.
...
|                                    | 22.04   | Fully Supported |                                                             |
|                                    | 24.04   | Fully Supported |                                                             |
|                                    | 26.04   | Fully Supported | Documentation was written using 26.04 LTS                   |
| **RHEL / Rocky Linux / AlmaLinux** | 8       | Fully Supported |                                                             |
|                                    | 9       | Fully Supported |                                                             |
| **Debian**                         | 11      | Fully Supported |                                                             |
|                                    | 12      | Fully Supported |                                                             |
|                                    | 13      | Fully Supported |                                                             |
| **Windows**                        | All     | Not Supported   | *Windows support will be added in future releases*.           |
| **MacOS** (M1 or Later)            | All     | Not Supported   | *MacOS support will be added in future releases*.             |

## System Requirements

To run Agent, you will need a Linux system capable of running Docker containers. Most VPS and almost all
...
run Agent. Some providers have made the necessary changes for nested virtualization to support Docker. Ask your provider's support team to make sure. KVM is guaranteed to work.
...
```bash
admin@reviactyl:~$ sudo dmidecode -s system-manufacturer
VMware, Inc.
```
...
<Callout type="warning" title="Check your Kernel">
...
<Callout type="warning" title="OVH/SYS Servers">
...
Once you have installed Agent and the required components, the next step is to create a node on your installed Panel. Go to your Panel administrative view, select Nodes from the sidebar, and on the right side click Create New button.

After you have created a node, click on it and there will be a tab called Configuration. Copy the code block content, paste it into a new file called `config.yml` in `/etc/reviactyl` and save it.

Alternatively, you can click on the Generate Token button, copy the bash command and paste it into your terminal.

![configuration](/docs/configuration-agent.png)

<Callout type="info" title="SSL Certificates">
When your Panel is using SSL, the Agent must also have one created for its FQDN. See [Creating SSL Certificates](/tutorials/creating-ssl-certificates) documentation page for how to create these certificates before continuing.
</Callout>

### Starting Agent

To start Agent, simply run the command below, which will start it in a debug mode. Once you confirmed that it is running without errors, use `CTRL+C` to terminate the process and daemonize it by following the instructions below. Depending on your server's internet connection pulling and starting Agent for the first time may take a few minutes.

```bash
sudo agent --debug
```

You may optionally add the `--debug` flag to run Agent in debug mode.
...
![allocations](/docs/allocations-node.png)
