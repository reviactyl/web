...
- PHP `8.2` or `8.3` (recommended) with the following extensions: `cli`, `openssl`, `gd`, `mysql`, `PDO`, `mbstring`, `tokenizer`, `bcmath`, `xml` or `dom`, `curl`, `zip`, `intl`, and `fpm` if you are planning to use NGINX.
...
```bash
curl -Lo panel.tar.gz https://github.com/reviactyl/panel/releases/download/v2.2.5/panel.tar.gz
